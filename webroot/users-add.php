...
if (mh_request_is_method("POST")) {
	...
	$errors["username"] = mh_validate_username($added_user["username"], "username");
	$errors["first_name"] = mh_validate_name($added_user["first_name"], "first name");
	...
	if (empty($errors["username"]) && empty($errors["first_name"]) && empty($errors["last_name"]) && empty($errors["email"])) {
		...
		$statement->bindValue(":last_name", $added_user["last_name"], PDO::PARAM_STR);
